...
const docsNav = Object.keys(config.repos).reduce((carry, key) => {
	carry[titleCase(key)] = key;
	return carry;
}, {});

const buildDocs = async (url, handle) => {
	const docs = {}
		, nav = {}
		, indexNav = {};
	...
	for (const { name, path: p, type, download_url } of data) {
		if (type === 'file') {
			...
			if (p !== 'docs/index.md') {
				nav[md.data.title] = path.join('../', dir, file);
				indexNav[md.data.title] = path.join('./', dir, file);
			}
		} else {
...
const renderDocs = (route = '', docs, nav, indexNav) => {
	for (let [name, obj] of Object.entries(docs)) {
		...
		if (!obj.hasOwnProperty('content'))
			return renderDocs(_route, obj, nav, indexNav);

		console.log('Rending', _route);

		let toc;
		const content = md.render(obj.content, {
			tocCallback (_, _toc) { toc = _toc },
			isIndex,
		});
...
module.exports = async function build (handle) {
	...
	try {
		const { docs, nav, indexNav } = await buildDocs(`https://api.github.com/repos/${repo}/contents/docs`, handle);
		rimraf.sync(path.join(config.output, handle));
		renderDocs(handle, docs, nav, indexNav);
		update(handle);
	} catch (e) {
